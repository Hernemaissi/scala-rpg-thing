...
// handling action
case class UserRequest[A](jwt: JwtClaim, token: String, userId: String, request: Request[A]) extends WrappedRequest[A](request)
  ...
  override def invokeBlock[A](request: Request[A], block: UserRequest[A] => Future[Result]): Future[Result] =
    extractBearerToken(request) map { token =>
      authService.validateJwt(token) match {
        case Success(claim) => {
          claim.subject match {
            case Some(value) => block(UserRequest(claim, token, value, request))
